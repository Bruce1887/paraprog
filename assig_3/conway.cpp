/******************************************************
 ************* Conway's game of life ******************
 ******************************************************

 Usage: ./exec ArraySize TimeSteps

 Compile with -DOUTPUT to print output in output.gif
 (You will need FFmpeg for that; install it with
  sudo apt-get install ffmpeg)
 WARNING: Do not print output for large array sizes!
          or multiple time steps!
 ******************************************************/

#include <stdio.h>
#include <stdlib.h>
#include <sys/time.h>
#include <iostream>
#include <omp.h>
#include <cassert>

#define FINALIZE "\
ffmpeg -y -start_number 0 -i out%d.pgm output.gif\n\
rm *pgm\n\
"

static int **allocate_array(int N)
{
    int **array;
    int i, j;
    array = (int **)malloc(N * sizeof(int *));
    for (i = 0; i < N; i++)
        array[i] = (int *)malloc(N * sizeof(int));
    for (i = 0; i < N; i++)
        for (j = 0; j < N; j++)
            array[i][j] = 0;
    return array;
}

static void free_array(int **array, int N)
{
    int i;
    for (i = 0; i < N; i++)
        free(array[i]);
    free(array);
}

static void init_random(int **array1, int **array2, int N)
{
    int i, pos;

    for (i = 0; i < (N * N) / 10; i++)
    {
        pos = rand() % ((N - 2) * (N - 2));
        array1[pos % (N - 2) + 1][pos / (N - 2) + 1] = 1;
        array2[pos % (N - 2) + 1][pos / (N - 2) + 1] = 1;
    }
}

#ifdef OUTPUT
static void print_to_pgm(int **array, int N, int t)
{
    int i, j;
    char *s = (char *)malloc(30 * sizeof(char));
    sprintf(s, "out%d.pgm", t);
    FILE *f = fopen(s, "wb");
    fprintf(f, "P5\n%d %d 1\n", N, N);
    for (i = 0; i < N; i++)
        for (j = 0; j < N; j++)
            if (array[i][j] == 1)
                fputc(1, f);
            else
                fputc(0, f);
    fclose(f);
    free(s);
}
#endif

int main(int argc, char *argv[])
{
    int arr_size;  // array dimensions
    int timesteps; // time steps
    int num_threads;
    int **current, **previous; // arrays - one for current timestep, one for previous timestep
    int **swap;                // array pointer
    int nbrs;

    double time; // variables for timing
    struct timeval ts, tf;

    /*Read input arguments*/
    if (argc != 4)
    {
        fprintf(stderr, "Usage: ./exec ArraySize TimeSteps Threads\n");
        exit(-1);
    }
    else
    {
        arr_size = atoi(argv[1]);
        timesteps = atoi(argv[2]);
        num_threads = atoi(argv[3]);
        omp_set_num_threads(num_threads);
    }

    /*Allocate and initialize matrices*/
    current = allocate_array(arr_size);  // allocate array for current time step
    previous = allocate_array(arr_size); // allocate array for previous time step

    init_random(previous, current, arr_size); // initialize previous array with pattern

#ifdef OUTPUT
    print_to_pgm(previous, arr_size, 0);
#endif

    /*Game of Life*/
    gettimeofday(&ts, NULL);

	for (int t = 0 ; t < timesteps ; t++) {
		#pragma omp parallel for collapse(2) num_threads(num_threads) private(nbrs)
			for (int i = 1 ; i < arr_size-1 ; i++)
				for (int j = 1 ; j < arr_size-1 ; j++) {
					// std::cout << omp_get_thread_num() << " in nbrs section" << std::endl;					
					nbrs = previous[i+1][j+1] + previous[i+1][j] + previous[i+1][j-1] \
						+ previous[i][j-1] + previous[i][j+1] \
						+ previous[i-1][j-1] + previous[i-1][j] + previous[i-1][j+1];
					if (nbrs == 3 || ( previous[i][j]+nbrs == 3))
						current[i][j] = 1;
					else 
						current[i][j] = 0;
				}
		
		#ifdef OUTPUT
		print_to_pgm(current, arr_size, t+1);
		#endif
        
		// std::cout << omp_get_thread_num() << " in swap section" << std::endl;
		//Swap current array with previous array 
        assert(omp_get_thread_num() == 0); // only one thread should do the swap otherwise stinky stuff will happen
		swap = current;
		current = previous;
		previous = swap;
	}

    gettimeofday(&tf, NULL);
    time = (tf.tv_sec - ts.tv_sec) + (tf.tv_usec - ts.tv_usec) * 0.000001;

    printf("GameOfLife: Size %d Steps %d Time %lf\n", arr_size, timesteps, time);
#ifdef OUTPUT
    system(FINALIZE);
#endif
    free_array(current, arr_size);
    free_array(previous, arr_size);

    return 0;
}
